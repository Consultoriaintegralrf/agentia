# Privacidad y datos — Agentia

Agentia es software **self-hosted**: se instala en **tu propia cuenta de Cloudflare**, con **tus propias llaves**. Nadie más recibe, ve o guarda las conversaciones de tus clientes.
...
Todo vive en **tu** D1 y **tu** Vectorize, dentro de tu cuenta de Cloudflare:
...
Cuando instalas Agentia, **tú eres el responsable** de los datos personales de tus clientes; Agentia es solo la herramienta. Con eso en mente:
...
## 5. Si conectas un panel de control externo (opcional)

`CONTROL_PLANE_TOKEN` habilita `/api/*` para que un panel externo (por ejemplo, uno que administre varios bots a la vez) lea la API de conteos descrita arriba: **números, nunca conversaciones**. Si no quieres ni eso, no configures ese secret y el bot funciona igual.

---

Agentia se entrega **tal cual**, bajo licencia MIT y sin garantías (ver [LICENSE](./LICENSE)). El cumplimiento legal de tu operación es responsabilidad tuya.
