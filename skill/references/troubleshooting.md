...
| `D1 create ... already exists` | la base de datos ya existía | corre `pnpm wrangler d1 list`, copia el `database_id` real y pégalo en `wrangler.toml` (binding **DB**, `agentia_bot_db`) |
| `Vectorize ... already exists` | el índice ya existía | corre `pnpm wrangler vectorize list` y reutiliza `agentia_bot_kb` (no lo vuelvas a crear) |
...
```bash
pnpm wrangler d1 create agentia_bot_db
pnpm wrangler vectorize create agentia_bot_kb --dimensions=1024 --metric=cosine
```
...
| deploy-check: `Missing binding DB / KB / CATALOG` | la base de datos, el índice o el bucket no existen | crea el faltante: `wrangler d1 create agentia_bot_db`, `wrangler vectorize create agentia_bot_kb --dimensions=1024 --metric=cosine`, o `wrangler r2 bucket create <nombre>` y verifica el binding en `wrangler.toml` |
...
| `Vectorize: index not found` | el índice no existe | `pnpm wrangler vectorize create agentia_bot_kb --dimensions=1024 --metric=cosine` |
